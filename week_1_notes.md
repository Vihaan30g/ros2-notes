...
#### **ROS 2  Action Components:**
    ...
    • Feedback - Updates while the task is in progress
    • Result - Final output when the task completes
    • Cancel - Option to cancel the task
    • Action Server - The part that performs the task
    • Action Client - The part that requests the task and listens for updates
## **6. Parameters**
